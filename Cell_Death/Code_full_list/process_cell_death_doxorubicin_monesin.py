...
for filename in all_files:
    if "mone" not in filename: continue
...
for fileprefixname in fileprefixname_list:
    ...
    for filename in all_files:
        ...
        for i_x in range(0, total_x):
            for i_y in range(0, total_y):
                ...
                if "doxo" in fileprefixname:
                    percentile = np.percentile(original[:, :, args.first_channel], 50) + 10
                elif "mone" in fileprefixname:
                    percentile = np.percentile(original[:, :, args.first_channel], 80) + 20
                ...
                contours = [cnt for cnt in contours if cv2.contourArea(cnt) > 50 and cv2.contourArea(cnt) < 2000]
                ...
                for contour in contours:
                    ...
                    area_bright = np.float32(first_channel > 200).sum()
                    if area_bright > 25:
                        continue
    ...
    name_first_split, current_concen = fileprefixname.split("concen")
    current_concen = current_concen[:1]
    
    # Save the data of the current prefix
    if (name_first_split not in max_concen) or (max_concen[name_first_split] < current_concen):
        max_concen[name_first_split] = current_concen
...
for drug_idx in range(2):
    ...
    with open(f"output_drug_{drug_names[drug_idx]}.csv", "w") as fout:
        for line in lines:
            fout.write(line + "\n")
