...
//		}
		...
		int percentile_value = get_percentile(tmp, tmpcnt, 65, true) + 65;
		...
		for (int k = 1; k <= tail; k++)
		{
			...
			for (int k2 = 1; k2 <= tail2 && Touch_boundary == false; k2++)
			{
				...
				for (int add_x = -5; add_x <= 5 && Touch_boundary == false; add_x++)
					for (int add_y = -5; add_y <= 5 && Touch_boundary == false; add_y++)
					{
						int next_x = cur_x2 + add_x;
						int next_y = cur_y2 + add_y;
