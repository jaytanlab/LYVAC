...
for fileprefixname in fileprefixname_list:
    ...
    for filename in all_files:
        ...
        for contour in contours:
            ...
            if args.mode == "single_total":
                ...
                first_channel = cell_pixels[:, :, args.first_channel]
                cell_intensity = np.percentile(first_channel[cell_pixels[:, :, args.first_channel] > 0], 20)
                cell_intensity_max = np.percentile(first_channel[cell_pixels[:, :, args.first_channel] > 0], 97)
                inside_cell = (cell_pixels[:, :, args.first_channel] > cell_intensity) & (cell_pixels[:, :, 0] < 25) \
    ...
    if fileprefixname == "20230312 slides 20230311 U2OS WT+OSBP-PH-GFP L1M594 DAPI_Control-":
        intensity1 = all_intensity
    if fileprefixname == "20230312 slides 20230311 U2OS WT+OSBP-PH-GFP L1M594 DAPI_Api30'-":
