...
for filename in all_files:
    if filename.endswith(('.png', '.tif', '.jpg')): # lossless format
        ...
        true_name = true_name.replace("2AKO3", "2ako").replace("2AKO5", "2ako")
        true_name = true_name.replace("2ako3", "2ako").replace("2ako5", "2ako")
        true_name = true_name.replace("control", "Control")
        
        if true_name not in fileprefixname_list:
            fileprefixname_list.append(true_name)
fileprefixname_list.sort()
# fileprefixname_list.reverse()
...
for fileprefixname in fileprefixname_list:
    ...
    for filename in all_files:
        ...
        true_name = filename.replace("2AKO3", "2ako").replace("2AKO5", "2ako")
        true_name = true_name.replace("2ako3", "2ako").replace("2ako5", "2ako")
        true_name = true_name.replace("control", "Control")
        
        if not true_name.startswith(fileprefixname): continue
        ...
        for contour in contours:
            ...
            if args.mode == "single_total":
                ...
                if len(first_channel[inside_cell]) == 0:
                    persentile_intensity = 120
                else:
                    if "Hypo" in fileprefixname:
                        percentile_intensity = np.percentile(first_channel[inside_cell], 80) + 45
                    else:
                        percentile_intensity = np.percentile(first_channel[inside_cell], 90) + 60
                # Calculate the valid mask of the first channel within the cell
            ...
            if args.mode == "single_total":
                ...
                if np.sum(np.float32(second_channel_mask)) < 10: continue
